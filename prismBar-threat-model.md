...
prismBar is a single-user, local-only macOS 27 utility with one intentionally powerful authority: Accessibility access to observe and move menu bar items. The highest-risk areas are the signed host boundary, the reciprocal XPC signing boundary, stale menu bar geometry, and release provenance. The code currently limits these risks with live permission checks, exact signing requirements, bounded parsing and execution, fresh topology verification, a sandboxed plugin service, no network stack, public-safety release audits, and a source-matched lifecycle stress run. No critical or high residual threat was identified in the current source. A current-source Developer ID archive, notarization, final shipping-artifact reconciliation, and physical Accessibility testing remain release gates.
...
- Preserved Developer ID archives for version 0.1.0 build 1 passed the release-bundle and reciprocal signing-boundary audits on 2026-08-26. The authentic host and embedded service were accepted, and identically named ad-hoc impostors were rejected in both directions.
- Preserved archives prove the implemented signing boundary but are never treated as the final artifact by documentation alone. Revision-bound candidate evidence, dual app and disk-image notarization, stapling, public-source binding, and the physical permission matrix remain release gates until their exact artifacts pass.
...
- **Build and release gates:** Pin tools, reject dependencies and sensitive artifacts, scan Git history, analyze code, run sanitizers from temporary directories outside the package root, audit the unsigned Release bundle, and verify reciprocal signing on a Developer ID archive. `scripts/ci-verify.sh`, `scripts/audit-public-safety.sh`, `scripts/audit-release-bundle.sh`, `scripts/audit-live-signing-boundaries.sh`.
...
| TM-002 | Local impersonating process | Attacker can start a process but lacks the project team certificate | Impersonate the host or bundled plugin over XPC | Untrusted commands or panel responses | Signing identity, plugin mutations | Reciprocal exact bundle and team requirements in `App/PluginClient.swift` and `XPC/prismCalcPluginService/ServiceMain.swift`; `scripts/audit-live-signing-boundaries.sh` accepted the authentic Developer ID pair and rejected ad-hoc impostors in both directions | Final notarized-candidate proof remains pending | Repeat the reciprocal audit on the final source-matched notarization candidate | Record only the pass or fail result and artifact digest in release evidence | Low | High | Medium |
| TM-003 | Malformed or hostile XPC payload | A valid peer is compromised or has a bug | Send oversized, malformed, recursive, or capability-expanding data | Host or service denial of service, unsafe rendering | Plugin availability, panel integrity | 64 KiB wire cap, typed Codable schema, bounded collections, URL rejection, allowlisted mutations, service sandbox, two-second timeout; five source-matched lifecycle cycles passed | Codable nesting depth depends on Foundation implementation | Keep hostile corpus, sanitizer, and lifecycle stress gates on release candidates | Test-only counters for timeout, decode rejection, and failure-budget transitions | Low | Medium | Low |
