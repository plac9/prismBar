...
- On 2026-09-03, the production-usability branch passed the complete unsigned verification matrix: Debug and Release package tests, physical macOS 27 app tests, Address Sanitizer, Thread Sanitizer, static analysis, warnings-as-errors builds, release-bundle allowlists, licensing, public-safety, Git-history secret scanning, and native-glass structure. An eleven-surface privacy-safe UI audit was visually reviewed, and a ten-cycle native prismDeck open/dismiss soak returned to the baseline app-owned window count every time.
- The installed-process resource gate is limited to the exact `/Applications/prismBar.app` executable and persists only aggregate CPU, footprint, thread-count, and growth values. It does not collect the command line, environment, other processes, windows, Accessibility values, or user content.
...
| Local preferences | App lifecycle and reading-size control | User profile to host | Reading size and request history are non-authoritative; malformed reading size restores Standard | `App/AppModel.swift`, `Sources/prismBarCore/PrismTextSizePreference.swift`, `App/Design/PrismTypography.swift`, `Sources/prismBarEngine/AccessibilityPermission.swift` |
...
| TM-008 | Local user-profile modification | Attacker can write the app preference domain | Change non-authoritative presentation or request history | Confusing UI state | Preferences, availability | Live AX trust is authoritative, stored state never grants permission, and reading size is a bounded enum that falls back to Standard | Preference tampering is not separately reported | Keep security decisions out of `UserDefaults`; continue normalizing malformed preferences to documented defaults | UI reflects current live permission state | Medium | Low | Low |
