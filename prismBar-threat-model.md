...
- **Plugin host:** Opens a named XPC service, enforces the plugin signing requirement, validates its manifest and declarative response, and applies an allowlisted local mutation. `App/PluginClient.swift`, `Sources/prismPluginKit/BundledPluginPolicy.swift`.
...
- **Host to macOS Accessibility:** The host reads menu bar roles, descriptions, titles, frames, and enabled state through AX APIs. Live `AXIsProcessTrusted` checks, per-application messaging timeouts, depth limits, and item-count limits bound this boundary.
...
- **Plugin response to local side effect:** A validated response may copy a bounded result to the pasteboard or open only the allowlisted prismCalc bundle identifier. Both require an explicit user command.
...
| Accessibility observations | Running local applications | App process to host | 256 apps, depth 4, 256 elements per app, 250 ms AX timeout | `Sources/prismBarAccessibility/NativeMenuBarObservationReader.swift` / `NativeMenuBarObservationReader` |
...
| App launch mutation | Explicit Open prismCalc command | Host to Launch Services | Exact allowlisted bundle identifier | `App/AppModel.swift` / `applyPluginMutations` |
...
5. A compromised plugin response tries to render a URL, huge value, duplicate control, excessive collection, or arbitrary app launch. The host rejects the entire response before rendering or applying mutations.
