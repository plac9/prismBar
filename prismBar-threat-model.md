...
- The preserved Developer ID archive for version 0.1.0 build 1 passed the release-bundle and reciprocal signing-boundary audits on 2026-08-26. The authentic host and embedded service were accepted, and identically named ad-hoc impostors were rejected in both directions.
- That archive predates source revision `5dc0be6`. It proves the implemented signing boundary but is not the final source-matched release artifact. A new archive, notarization, source-revision binding, and physical permission matrix remain release gates.
